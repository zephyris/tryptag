  ...
  def _fetch_zenodo_record_file(self, zenodo_json, file_name: str) -> str:
    ...
    for file in zenodo_json["files"]:
      if file["key"] == file_name:
        url = f"https://zenodo.org/api/records/{zenodo_json['id']}/files/{file_name}/content"
        ...
        if "record_id" not in self.zenodo_index[plate]:
          ...
          for file in zenodo_json["files"]:
            if file["key"].endswith("_processed.zip"):
              self.zenodo_index[plate]["record_url"] = f"https://zenodo.org/api/records/{zenodo_json['id']}/files/{file['key']}/content"
...
class BSFTag(TrypTag):
  def __init__(self, *args, **kwargs):
    ...
    super().__init__(self, *args, **kwargs, master_zenodo_id=7258722, data_cache_path=data_cache_path, data_cache_plates=4, life_stages=["bloodstream"])
