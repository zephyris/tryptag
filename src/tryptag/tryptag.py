...
class CellLine:
    """
    CellLine object holding information on a cell line dataset - life cycle
    stage, tagged gene and tagged terminus.
    """
...
class TrypTag:
    def __init__(
        ...
        remove_zip_files -- whether to remove zip files after download and
          extraction (default `True`, doubles data cache size if `False`)
        master_zenodo_id -- Zenodo ID of the master TrypTag deposition
          (default `6862289`, do not change unless you know what you're doing)
        data_cache_plates -- Number of plates in data cache
          (default chosen for TrypTag's 222 plates)
        data_cache_platesize -- Size (in bytes) of one plate directory
          (default chosen for TrypTag's plates, 17 Gb)
        data_cache_zipsize -- Size (in bytes) of one plate zip file
          (default chosen for TrypTag's plates, 14 Gb)
        ...
        # user setting: remove zip files after download (~doubles data cache
        # size if False)
        ...
        self._data_cache_size = data_cache_plates * \
            data_cache_platesize + data_cache_zipsize
    ...
    def _fetch_zenodo_text(self, url: str, use_file_cache: bool = True) -> str:
        """
        Data retrieval helper function. Fetch a string from any Zenodo URL,
        respecting rate limits. Caches data as text files in `_zenodo`
        subdirectory of `data_cache_path`. Also caches data in memory in
        `self._url_str_cache` to boost performance.
        ...
        file_cache_path = os.path.join(
            zenodo_cache_path, url[len("https://"):].replace("/", "-")+".txt")
        ...
        while response is None:
            try:
                ...
                if self.print_status:
                    print("  Zenodo query rate limit reached, waiting to"
                          " retry")
                # MAGIC NUMBER: testing shows the rate limiter resets after
                # 50s, so 60s for a bit of space
                time.sleep(60)
        text = (
            response
            .read()
            .decode(
                response.info().get_param("charset") or
                "utf-8-sig"
            )
        )
        # cache in memory
        ...
        if not os.path.isdir(zenodo_cache_path):
            if self.print_status:
                print("  making zenodo cache directory: "+zenodo_cache_path)
    ...
    def _fetch_zenodo_record_json(self, zenodo_id: str):
        ...
        url = "https://zenodo.org/api/records/" + \
            str(zenodo_id)+"/versions/latest"
        if self.print_status:
            print("  Fetching Zenodo record for "+str(zenodo_id)+" from: "+url)
        text = self._fetch_zenodo_text(url)
        return json.loads(text)

    def _fetch_zenodo_record_file(self, zenodo_json, file_name: str) -> str:
        """
        Data retrieval helper function. Fetch the contents of a text file in a
        Zenodo ID.
        ...
        """
        deposition_root = f"{ZENODO_API_ROOT}{zenodo_json['id']}/files/"
        for file in zenodo_json["files"]:
            if file["key"] == file_name:
                url = f"{deposition_root}{file_name}/content"
                if self.print_status:
                    print("  Fetching file "+file_name+" from: "+url)
    ...
    def _parse_localisation_annotation(self, string: str) -> list:
        """
        Parse standard localisation annotation string in the form
        term[modifier,modifier],term,term[modifier] etc.

        :return: List of dicts containing localisation terms and modifiers in
          the form `{"term": term, "modifiers": [modifier, modifier]}`
        """
        import re
        # Regex to extract the term and modifiers from strings such as
        # "term[modifier1,modifier2]"
        annotation_re = re.compile(
            r'^(?P<term>[^[\]]+)(?:|\[(?P<modifiers>[^\]]+)\])$')
        annotation_list = []
        # Split a string into comma-separated parts, skipping commas that are
    ...
    def zenodo_index(self) -> list:
        ...
        lines = self._fetch_zenodo_record_file(
            zenodo_json, "plate_doi_index.tsv").splitlines()
        # parse tsv to dict indexed by plateID_YYYYMMDD
        for line in lines:
            line = line.split("\t")
            zenodo_index[line[1]] = {
                "master_record_id": line[0].split(".")[-1]}
        return zenodo_index

    @cached_property
    def gene_list(self) -> list:
        """
        Master data list, organised as dicts within
        `gene_list[life_stage][gene_id][terminus]` structure.
        ...
        if self.print_status:
            print("Fetching gene list from Zenodo, record ID: " +
                  str(self.master_zenodo_id))
        if self.print_status:
            print("  Using latest Zenodo version, record ID: " +
                  self.zenodo_record_id)
        # load localisations table
        # download localisations.tsv from master record
        zenodo_json = self._fetch_zenodo_record_json(self.zenodo_record_id)
        lines = [line for line in self._fetch_zenodo_record_file(
            zenodo_json, "localisations.tsv").splitlines() if line.strip()]
        # load into default life stage
        # TODO: Handle loading of multiple different life stages
        gene_list = {self.life_stages[0]: {}}
        # parse line by line, expects the first line to be headers and grab
        # indices
        for line in lines:
            line = line.split("\t")
            if line[0] == "Gene ID":
                indices = {}
                for line_nr in range(len(line)):
                    indices[line[line_nr]] = line_nr
            else:
                ...
                for t in termini:
                    ...
                    if line[indices[p + "status"]] == "cell line generated":
                        ...
                        plate, well = (
                            line[indices[p + "plate and well"]]
                            .split(" ")
                        )
                        localisation = self._parse_localisation_annotation(
                            line[indices[p + "localisation"]])
                        terminus_data = {
                            "plate": plate,
                            "well": well,
                            "loc": localisation,
                        ...
                        if p+"classified as faint" in indices:
                            terminus_data["signl_low"] = (
                                line[indices[p + "fainter than parental"]])
                            terminus_data["signal_background"] = (
                                line[indices[p + "classified as faint"]])
                        # look up zenodo id
                        index = self.zenodo_index[terminus_data["plate"]]
                        terminus_data["zenodo_id"] = index["master_record_id"]
                        gene_list[self.life_stages[0]
    ...
    def worklist_all(self, life_stage: str = None) -> list:
        ...
        genes_termini = (
            ...
            for terminus in self.termini if terminus in gene_entry
        )
        return [
            CellLine(
    ...
    def localisation_ontology(self) -> list:
        """
        Localisation ontology annotation terms for intelligent localisation-
        based searching.
        """

        def _parse_localisation_ontology(
                ontology_json,
                parent: list = ["root"]
        ) -> dict:
            """
            Parse localisaiton ontology from hierachical object to list of
            objects with parent and children lists. Recursively called to
            assemble list.

            :param ontology_json: Parsed ontology JSON.
            :param parent: List of parent terms, used in recursive calls to
              build ontology.
            :return: List of dicts, each containing information about that
              localisation ontology term.
            """
            terms = {}
            ...
            for loc_nr in range(len(loclist)):
                # add current entry to terms dict
                terms[loclist[loc_nr]["name"]] = loclist[loc_nr].copy()
                terms[loclist[loc_nr]["name"]]["parent"] = parent
                if "sublocalisation" in loclist[loc_nr]:
                    # record children to current entry
                    terms[loclist[loc_nr]["name"]]["children"] = []
                    del terms[loclist[loc_nr]["name"]]["sublocalisation"]
                    for s in range(len(loclist[loc_nr]["sublocalisation"])):
                        terms[loclist[loc_nr]["name"]]["children"].append(
                            loclist[loc_nr]["sublocalisation"][s]["name"])
                    # recurse through subterms
                    terms.update(
                        _parse_localisation_ontology(
                            loclist[loc_nr],
                            parent=parent + [loclist[loc_nr]["name"]]
                        )
                    )
            ...
            if parent == ["root"]:
                terms["root"] = {
                    ...
                    "children": [
                        x["name"]
                        for x in ontology_json["localisation"]
                    ]
                }
            return terms
        ...
        return _parse_localisation_ontology(
            json.loads(
                self._fetch_zenodo_record_file(
                    zenodo_json,
    ...
    def _localisation_match(
        ...
        """
        Test if the localisation annotation of `life_stage`, `gene_id` and
        `terminus` in `gene_list` match the query.
        ...
        for loc_nr in range(len(localisations)):
            # check if any necessary modifiers are present
            modifiers_included = True
            if required_modifiers is not None:
                modifiers_count = 0
                if "modifiers" in localisations[loc_nr]:
                    for modifier in required_modifiers:
                        if modifier in localisations[loc_nr]["modifiers"]:
                            modifiers_count += 1
                if modifiers_count < len(required_modifiers):
                    modifiers_included = False
            if not modifiers_included:
                break
            # check if the current localisation term should be exlcuded from
            # matching based on modifiers
            modifiers_excluded = False
            if (
                "modifiers" in localisations[loc_nr] and
                exclude_modifiers is not None
            ):
                for modifier in exclude_modifiers:
                    if modifier in localisations[loc_nr]["modifiers"]:
                        modifiers_excluded = True
                        break
            # if not excluded based on modifiers, search for an annotation
            # query_term match
            if modifiers_excluded:
                break
            # exact match
            if localisations[loc_nr]["term"] == query_term:
                return True
            # parent match, if matching subterms
            term = self.localisation_ontology[localisations[loc_nr]["term"]]
            if match_subterms and query_term in term["parent"]:
                return True
    ...
    def check_data_cache_usage(self, exact: bool = False):
        ...
        Prints information about disk usage for data cache, free space and
        whether the full cache will fit.

        :param exact: If `True`, then calculate usage exactly. Otherwise,
            approximate estimate from number of directories.
        """
        if exact:
            ...
            for file in os.listdir(self.data_cache_path):
                if os.path.isfile(os.path.join(self.data_cache_path, file)):
                    sum_zip += os.path.getsize(
                        os.path.join(self.data_cache_path, file))
                elif os.path.isdir(os.path.join(self.data_cache_path, file)):
                    for subfile in os.listdir(
                        os.path.join(self.data_cache_path, file)
                    ):
                        sum_dir += os.path.getsize(os.path.join(
                            self.data_cache_path, file, subfile))
        else:
            # quick approximation, count directories and multiply by expected
            # size
            str_prefix = "~"
            sum_dir = 0
            sum_zip = 0
        ...
        if self.print_status:
            ...
            print("  Directory usage: "+str_prefix +
                  str(round(sum_dir / float(2 << 40), 4))+" TiB")
            print("  Zip file usage: "+str_prefix +
                  str(round(sum_zip / float(2 << 40), 4))+" TiB")
        # check disk space
        space_required = self._data_cache_size
        if not self.remove_zip_files:
        ...
        if free < space_required:
            if self.print_status:
                print(
                    "! Insufficient free disk space for full data cache: " +
                    str(round(free / float(2 << 40), 4)) +
                    " / " + str(round(space_required / float(2 << 40), 4)) +
                    " TiB available !"
                )

    def _count_cells(self, cell_line):
        """
        Counts the number of fields of view and cells in them for for the
        requested cell_line (`terminus`, `gene_id`, `terminus`) from the data
        cache. Records the data in `self.gene_list`, as `fields_count` `int`,
        `cells_per_field` `list` and `cells` `list` of `list`.
        ...
        if "cells" not in terminus and os.path.isdir(base_path):
            ...
            files = glob.glob(
                os.path.join(
                    ...
                    (
                        cell_line.gene_id +
                        "_4_" +
                        cell_line.terminus.upper() +
                        "_*_roisCells.txt"
                    )
                )
            )
            for filename in files:
                with open(filename) as cells_file:
                    lines = [x.strip().split("\t")
                             for x in cells_file.readlines()]
                    # filter rare misformated lines, eg. BSF
                    # Tb427.03.2580_4_C_4_roisCells.txt
            ...
            terminus["fields_count"] = len(cells)
            terminus["cells_per_field"] = [len(x) for x in cells]
            terminus["cells"] = cells.copy()
            if self.print_status:
                print(
                    "  Counted", terminus["fields_count"],
                    "image data files with",
                    sum(terminus["cells_per_field"]), "cells for",
                    cell_line.life_stage, cell_line.gene_id, cell_line.terminus
                )

    def fetch_data(self, cell_line):
        """
        Downloads and caches microscopy data for the plate containing a
        `gene_id` and `terminus` combination. Places data in
        `self.data_cache_path`. Updates
        self.gene_list[life_stage][gene_id][terminus] with information
        about number of fields of view and cells

        :param cell line: `CellLine` object containing `life_stage`, `gene_id`
            and `terminus`.
        :return: List of dicts of all cells for this `life_stage`, `gene_id`
            and `terminus` in the form
        ...
        try:
            if (
                not os.path.isfile(zip_path) and
                not os.path.isfile(
                    os.path.join(dir_path, "_"+plate+".zip.md5"))
            ):
                self.check_data_cache_usage()
                print("Fetching data for " +
                      cell_line.life_stage+" form, gene ID " +
                      cell_line.gene_id+" tagged at " +
                      cell_line.terminus+" terminus")
                # fetch the processed microscopy data from Zenodo
                if self.print_status:
                    print("  Making plate data directory for: "+plate)
                if "record_id" not in self.zenodo_index[plate]:
                    # if not already translated, fetch the latest Zenodo ID
                    # from master Zenodo ID, fetch Zenodo record JSON,
                    # to get latest version doi
                    zenodo_json = self._fetch_zenodo_record_json(
                        terminus["zenodo_id"])
                    self.zenodo_index[plate]["record_doi"] = zenodo_json["doi"]
                    self.zenodo_index[plate]["record_id"] = str(
                        zenodo_json["id"])
                    root = f"{ZENODO_API_ROOT}{zenodo_json['id']}"
                    for file in zenodo_json["files"]:
                        if file["key"].endswith("_processed.zip"):
                            self.zenodo_index[plate]["record_url"] = (
                                f"{root}/files/{file['key']}/content")
                            self.zenodo_index[plate]["record_md5"] = (
                                file["checksum"].split(":")[-1])
                # download data
                zip_md5 = 0
                zip_path_temp = os.path.join(
                    self.data_cache_path, plate+".zip.tmp")
                while zip_md5 != self.zenodo_index[plate]["record_md5"]:
                    ...
                    if self.print_status:
                        print("  Checking MD5 hash of: "+plate+".zip.tmp")
                    zip_md5 = _file_md5_hash(zip_path_temp)
                    if zip_md5 != self.zenodo_index[plate]["record_md5"]:
                        if self.print_status:
                            print("! MD5 of downloaded zip is incorrect (" +
                                  zip_md5+"), retrying download !")
                if self.print_status:
            ...
            try:
                if not os.path.isfile(
                    ...
                    # unzip data
                    if self.print_status:
                        print("  Decompressing image data from: "+plate+".zip")
                    os.mkdir(dir_path)
                    try:
                        with ZipFile(zip_path) as archive:
                            ...
                            for file in tqdm(
                                ...
                                miniters=1,
                                desc=plate,
                                disable=not self.print_status,
                                smoothing=0
                            ):
                                # loop through all files, finding files ending
                                # with the cell roi suffix and not starting
                                # with control (or common misspellings)
                                fname_end = os.path.split(file)[-1]
                                if (
                                    file.endswith(suffix) and
                                    not (
                                        fname_end.startswith("control") or
                                        fname_end.startswith("ontrol") or
                                        fname_end.startswith("Control")
                                    )
                                ):
                                    # infer main tif and thresholded tif image
                                    # filenames
                                    file_roi = file
                                    file_tif = file[:-len(suffix)]+".tif"
                                    file_thr = file[:-len(suffix)]+"_thr.tif"
                                    if (
                                        file_roi in archive.namelist() and
                                        file_tif in archive.namelist() and
                                        file_thr in archive.namelist()
                                    ):
                                        count_decompressed += 1
                                        # if all exist, decompress and move to
                                        # plate directory
                                        archive.extract(
                                            file_roi, self.data_cache_path)
                                        archive.extract(
                                            file_tif, self.data_cache_path)
                                        archive.extract(
                                            file_thr, self.data_cache_path)
                                        target_path = os.path.join(
                                            self.data_cache_path, plate)
                                        shutil.move(
                                            os.path.join(
                        ...
                        if self.print_status and len(missing) > 0:
                            print(
                                "! Expected files were missing in the zip for "
                                "the following gene IDs !")
                            print(" ".join(missing))
                        # remove any subdirectories remaining from
                        # decompression
                        obj = os.scandir(os.path.join(
                            self.data_cache_path, plate))
                        for entry in obj:
                            if entry.is_dir():
                                shutil.rmtree(os.path.join(
                                    self.data_cache_path, plate, entry.name))
                        obj.close()
                        # copy source zip md5 to the data directory, also
                        # marks decompression as complete
                        shutil.copyfile(
                            zip_path+".md5",
                            os.path.join(dir_path, "_"+plate+".zip.md5")
                        )
                        ...
                        if self.print_status:
                            print("  Decompressed " +
                                  str(count_decompressed)+" fields of view")
                    except BadZipFile:
    ...
    def cell_list(self, cell_line) -> list:
        """
        Returns a list fields of view and cells in them for for the requested
        cell_line (`terminus`, `gene_id`, `terminus`).

        :param cell line: `CellLine` object containing `life_stage`, `gene_id`
            and `terminus`.
        :return: List of dicts of all cells for this `life_stage`, `gene_id`
            and `terminus` in the form `{"field_index": field_index,
    ...
    def check_data_cache_integrity(self):
        """
        Checks all microscopy in the data cache, reporting any missmatches of
        MD5 hash per data plate. Any MD5 hash missmatches imply a new version
        of data for that plate may be available. Checks the number of images
        available per cell line for obvious missing data. Does not do a full
        check and may well throw `FileNotFoundError` if data cache is very
        malformed. Temporarily overrides `self.print_status` to `False` so
        only its specific output is shown.
        """
        import os
        ...
        for plate in self.zenodo_index:
            ...
            if os.path.isdir(dir_path) or os.path.isfile(zip_path):
                ...
                zenodo_json = self._fetch_zenodo_record_json(
                    self.zenodo_index[plate]["master_record_id"])
                for file in zenodo_json["files"]:
                    if file["key"].endswith("_processed.zip"):
                        self.zenodo_index[plate]["record_md5"] = (
                            file["checksum"].split(":")[-1]
                ...
                if not os.path.isfile(md5_file):
                    ...
                    with open(md5_file, "r") as file:
                        ...
                        if md5 != self.zenodo_index[plate]["record_md5"]:
                            print(
                                "  MD5 file in directory does not match "
                                "zenodo record md5:", plate)
            if os.path.isfile(zip_path):
                ...
                if not os.path.isfile(md5_file):
                    ...
                    with open(md5_file, "r") as file:
                        ...
                        if md5 != self.zenodo_index[plate]["record_md5"]:
                            print(
                                "  MD5 file for zip file does not match "
                                "zenodo record MD5:", plate)
            ...
            if os.path.isdir(dir_path):
                ...
                for life_stage in self.life_stages:
                    for cell_line in self.worklist_all(life_stage):
                        gene = self.gene_list[life_stage][cell_line.gene_id]
                        terminus = gene[cell_line.terminus]
                        if terminus["plate"] == plate:
                            if "cells" in terminus:
                                # ensure cells are counted
                                self.cell_list(cell_line)
                                if len(terminus["cells"]) == 0:
                                    print("  No images found, but images "
                                          "expected, for",
                                          life_stage, cell_line.gene_id,
                                          terminus, "in", plate)
    ...
    def _open_field(
        ...
        :return: List with one `skimage` image per image channel and threshold
            image. List is in the order
            `[phase, mng, dna, phase_mask, dna_mask]`.
        """
        ...
        field_base_path = os.path.join(
            self.data_cache_path,
            terminus["plate"],
            cell_line.gene_id + "_4_" +
            cell_line.terminus.upper() +
            "_"+str(field_index + 1)
        )
        if field_base_path != self._field_base_path_sk:
            # if not the last path opened, open field and threshold images and
            # store base path in self
            ...
            field_threshold = skimage.io.imread(
                self._field_base_path_sk+"_thr.tif")
    ...
    def open_field(self, cell_line, field_index: int = 0) -> list:
        ...
        :return: FieldImage object, containing the image channels as
            ...
            and `dna_mask`.
        """
        [phase, mng, dna, phase_mask, dna_mask] = self._open_field(
            cell_line, field_index)
        return FieldImage(
            phase=phase,
            mng=mng,
            dna=dna,
    ...
    def _open_cell(
        ...
        """
        Returns cell image data, returning a list with one `skimage` image per
        image channel and threshold image.

        :param cell line: `CellLine` object containing `life_stage`, `gene_id`
            and `terminus`.
        :param field_index: Index of the field of view. If not set, then `0`.
        :param crop_centre: `(x, y)` tuple around which to crop. Ignored if
            `width <= 0`.
        :param fill_centre: `(x, y)` at which to do a flood fill to select the
            target cell object in phase_mask.
        :param custom_field_image: `FieldImage` object containing custom field
            images to use. Images can be skimage image or `None`. Entries of
        ...
        :param width: If positive, width of cropped cell image centred on
            `crop_centre`. If negative, padding around the `phase_mask`.
            Default, `323`.
        :return: List with one `skimage` image per image channel and threshold
            image. List is in the order
            `[phase, mng, dna, phase_mask, dna_mask, phase_mask_othercells]`.
        """
        ...
        channels = self._open_field(
            cell_line, field_index, custom_field_image=custom_field_image)
        ...
        channels[3] = skimage.morphology.flood_fill(
            channels[3], (fill_centre[1], fill_centre[0]), 255)
        # append a copy (channels index 5), pixels if equal 127 ie. other cells
        channels.append(255*(channels[3] == 127))
        # filter channels index 3 for current cell only, set pixels equal to
        # 127 to zero
    ...
    def open_cell_custom(
        self,
        cell_line,
        field_index: int = 0,
        cell_index: int = None,
        custom_field_image=None,
        fill_centre: tuple = None,
        crop_centre: tuple = None,
        rotate: bool = False,
        angle: float = None,
        width: int = 323,
    ) -> list:
        """
        Advanced customisable open cell, opens a cell from a `gene_id`,
        `terminus` and `field_index`, but with customisable images, cell
        coordinates and/or angle. This allows use of custom pth image
        and cell coordinates, default cell coordinates but prefiltered
        mng image, etc.

        :param cell line: `CellLine` object containing `life_stage`, `gene_id`
            and `terminus`.
        :param field_index: Index of the field of view. If not set, then `0`.
        :param cell_index: Index of the cell in the field of view, used for
            finding default `fill_centre` and `crop_centre` coordinates.
            If not set or `None`, then use `fill_centre` coordinate.
        :param custom_field_image: `FieldImage` object containing custom field
            images to use. Images can be skimage image or `None`. Entries of
            None will use tryptag default. If not set or `None`, then use all
            tryptag defaults.
        ...
        :param width: If positive, width of cropped cell image (may clip very
            ...
            `phase_mask`. Default, `323`.
        :return: List with one `skimage` image per image channel and threshold
            image. List is in the order `[phase_(gray), mng_(green),
        ...
        if cell_index is not None:
            ...
            if fill_centre is None:
                raise ValueError(
                    "If `cell_index` is not set or `None` then `fill_centre` "
                    "must be set and not be `None`")
            # if custom fill_centre, check for crop_centre otherwise default
            # to fill_centre
            if crop_centre is None:
    ...
    def analyse_list(
        ...
        if multiprocess_mode is None:
            ...
            if self.print_status:
                print("  Parallel processes with", workers, "workers")
            Executor = concurrent.futures.ProcessPoolExecutor
        elif multiprocess_mode == "thread":
            # setup executor as a thread pool
            if self.print_status:
                print("  Parallel threads with", workers, "workers")
            Executor = concurrent.futures.ThreadPoolExecutor
        else:
            raise ValueError(f"Unknown multiprocess_mode '{multiprocess_mode}")

        # trigger fetch of gene_list and zenodo_index, prior to copying for
        # each thread
        self.zenodo_index
        self.gene_list
...
class BSFTag(TrypTag):
    def __init__(self, *args, **kwargs):
        ...
        if 'data_cache_path' in kwargs:
            data_cache_path = kwargs['data_cache_path']
            del kwargs['data_cache_path']
        else:
            data_cache_path = "./_bsftag_cache"
        super().__init__(
            self,
            *args,
            **kwargs,
            master_zenodo_id=7258722,
            data_cache_path=data_cache_path,
            data_cache_plates=4,
            life_stages=["bloodstream"]
        )
